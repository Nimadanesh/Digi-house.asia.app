...
async function main() {
  ...
  if (hasYieldWorker) {
    const properties = createDbPropertyStore(db);
    const yieldDeps = {
      locks: createDbShareLockStore(db),
      yields: createDbYieldStore(db),
    ...
    if (!queue) {
      log.fatal("failed to create yield queue");
      process.exit(1);
    }
    await scheduleYieldTickJobs(queue, env.YIELD_TICK_MS);
    log.info(
      { everyMs: env.YIELD_TICK_MS, maturationMs: env.UNLOCK_MATURATION_MS, queue: "digihouse-yield" },
      "scheduled repeatable tickYield",
    );

    const yieldHandle = startYieldWorker({
      redisUrl: env.REDIS_URL!,
      deps: yieldDeps,
