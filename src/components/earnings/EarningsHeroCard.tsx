"use client";
// File responsibility: Income hero (Phase 9 Slice 5 — UI Mapping §7.1/§7.3).
// Hero = "Received in total" (paid money only, §7.1) + next projected distribution
// with the status word Expected — never "guaranteed"/"Est." (§7.3 rules 1–4).
...
import { consecutivePaidWeeks } from "@/lib/earnings-stats";
...
export function EarningsHeroCard({ summary }: { summary: EarningsSummary }) {
  ...
  return (
    <Block className="p-4 space-y-4" data-testid="earnings-hero">
      ...
      {summary.thisWeekProjectedUsd > 0 ? (
        <div
        ...
        >
          ...
          <div className="shrink-0 text-end">
            <p className="text-[0.6875rem] leading-snug text-muted-foreground">{t("expected")}</p>
