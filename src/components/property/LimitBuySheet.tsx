...
// buy/sell-sheet convention (localized via next-intl property.*). A live buy
  ...
  function submit() {
    ...
    placeOrder.mutate(
      ...
      },
      {
        onSuccess: () => {
          onClose();
          pushToast("success", t("buyOrderPlacedToast"), t("buyOrderPlacedBody"));
        },
      },
    );
  }
      ...
      <div className="space-y-4 pb-2" data-testid="limit-buy-sheet">
        ...
        <Block>
          <Row className="!min-h-[48px]">
            <span className="text-sm text-muted-foreground">{t("bestAsk")}</span>
        ...
        <div className="rounded-[12px] bg-surface-2 p-3 space-y-1.5" data-testid="limit-buy-summary">
          ...
          <div className="flex justify-between text-sm">
            <span className="text-muted-foreground">
              {t("marketFeeLabel")}{tier ? ` (${bpsToPct(tier.buySecondaryBps)})` : ""}
            </span>
            <span className="tnum font-medium text-muted-foreground">{fee == null ? "—" : usd(fee)}</span>
          </div>
          <div className="flex justify-between text-sm border-t border-border pt-1.5">
            <span className="text-muted-foreground">{t("heldInEscrow")}</span>
