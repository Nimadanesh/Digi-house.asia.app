    ...
    <Sheet open={open} onClose={onClose} labelledBy="about-legal-title" className="max-h-[85svh] overflow-y-auto">
      <div className="space-y-4 pb-2" data-testid="about-legal-sheet">
        ...
        <div className="space-y-3 text-sm leading-relaxed text-muted-foreground">
          <p>{t("aboutBody")}</p>
          <p className="text-[0.6875rem] text-muted-foreground">{t("demoTxDisclaimer")}</p>
          <p className="text-[0.6875rem] text-muted-foreground">{t("payoutDisclaimer")}</p>
          <p>{t("aboutAdvice")}</p>
          <p className="text-xs text-muted-foreground">{t("aboutCopyright")}</p>
        </div>
      </div>
    </Sheet>
  );
